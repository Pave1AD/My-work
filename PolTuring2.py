    ...
    def merge(self, file1, file2, file1_last_state):
        ...
        for i in file2_dict.keys():
            if (i in file1_dict.keys()):
                ...
                output_file.write(i)
                output_file.write('\t'*(file1_last_state))
                line_list = file2_dict[i].split('\t')
                for i in range(len(line_list)):
                    output_file.write('\t')
                    if line_list[i] != '':
...
##Start##
file1_path = r"C:\Users\pavel\OneDrive\Рабочий стол\Turing machine tools\pr.txt"
file2_path = r"C:\Users\pavel\OneDrive\Рабочий стол\Turing machine tools\ttt.txt"
file1_last_state = 9
Turing = PolTuring()
