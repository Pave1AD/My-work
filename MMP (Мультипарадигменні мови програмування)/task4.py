...
class DataCleaner:
    ...
    def __init__(self, dataframe):
        ...
        self.history = [("Initial df", dataframe.copy())]

    def adjust_dtype(self, columns):
        for i, j in columns.items():
            self.current[i] = self.current[i].astype(j)
        self.history.append(("Adjusted dtypes using " + str(columns), self.current.copy()))

    # def adjust_dtype(self, columns):
    #     for i, j in columns.items():
    #         if j == np.datetime64:
    #             self.current[i] = pd.to_datetime(self.current[i])
    #         else:
    #             self.current[i] = self.current[i].astype(j)
    #     self.history.append(("Adjusted dtypes using " + str(columns), self.current.copy()))
    ...
    def impute_missing(self, columns):
        for i in columns:
            self.current[i].fillna(self.current[i].mean(), inplace=True)
        self.history.append(("Imputed missing in " + str(columns), self.current.copy()))
    ...
    def __del__(self):
        """
        Don't forget to clean files on the server after finishing
        """
        for file in os.listdir():
            if file.startswith("transactions") and file.endswith(".pkl"):
                os.remove(file)
